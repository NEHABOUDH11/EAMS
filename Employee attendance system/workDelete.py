...
def workplanfour():
    ...
    def finddata():
        ...
        cur=db.cursor()
        x=a.get()
        s="select Monday,Tuesday,Wednesday,Thursday,Friday,Saturday from wplan where planid = '%s' "%(x)
        cur.execute(s)
        data=cur.fetchone()
        e2.delete(0,100)
        e3.delete(0,100)
    ...
    def workDelete():
        ...
        s="delete wplan set Monday='%s',Tuesday='%s',Wednesday='%s',Thursday='%s',Friday='%s',Saturday='%s' where planid='%s'"%(b,c,d,e,f,g,h,x)
        cur.execute(s)
        db.commit()
